...
`,ve=new Cesium.PostProcessStage({fragmentShader:fe,uniforms:{time:()=>performance.now()/1e3%1e3,windDir:()=>P,rainSpeed:()=>N}});p.scene.postProcessStages.add(ve);function G(t){switch(t){case"east":P=new Cesium.Cartesian2(.5,0);break;case"west":P=new Cesium.Cartesian2(-.5,0);break;case"north":P=new Cesium.Cartesian2(0,-.5);break;case"south":P=new Cesium.Cartesian2(0,.5);break;default:P=new Cesium.Cartesian2(0,0)}}document.getElementById("windSlider").addEventListener("input",t=>{const e=parseInt(t.target.value,10);document.getElementById("windValue").textContent=e,M=e/200,v&&v.updateOptions({speedFactor:M})});document.getElementById("typhoonSlider").addEventListener("input",t=>{const e=parseInt(t.target.value,10);document.getElementById("typhoonValue").textContent=e,N=e/200});p.camera.flyTo({destination:Cesium.Cartesian3.fromDegrees(124.6,12.07,15e3)});let g={west:124.4,east:124.7,south:12.01,north:12.1},_=CesiumHeatmap.create(p,g,{maxOpacity:.6,minOpacity:.1,blur:.85}),k=[];for(let t=0;t<1e3;t++)k.push({x:g.west+Math.random()*(g.east-g.west),y:g.south+Math.random()*(g.north-g.south),value:Math.floor(Math.random()*100)});_.setWGS84Data(0,100,k);_._layer.show=!1;let V=_._layer;function xe(){V.show=!V.show}document.getElementById("toggleHeatmapBtn").addEventListener("click",xe);
